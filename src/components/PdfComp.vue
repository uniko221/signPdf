<template>
    <div class="flex ">
        <div class="bg-white w-1/4 h-screen fixed py-14 px-4 overflow-auto">
            <div v-for="(image, i) in signStores.signImages" :key="i" @click="addSign(image)">

                <img :src="signStores.signImages[i]" class="border" />

            </div>

            <button type="button" class="border  w-full text-center mt-5 py-4" @click="openNewSign">建立新簽名 </button>
        </div>

        <div class="pt-20 flex justify-center  w-full ">
            <input type="file" accept="application/pdf" @change="handleFile" />
            <!-- <button @click="downloadPDF" class="bg-blue-500 text-white px-4 py-2 rounded">下載 PDF</button> -->
            ...
            <div class="">
                ...
                <canvas v-for="(page, index) in pdfPages" :key="index" :ref="el => canvasRefs[index] = el"></canvas>
            </div>
    ...
    <footer class="bg-light-main p-4 fixed z-10 w-full bottom-0">
        ...
        <div class="w-full flex justify-between">
            ...
            <div><button @click="downloadPDF" class="bg-blue-500 text-white px-4 py-2 rounded">下載 PDF</button></div>
        </div>
    </footer>
...
//   // 建立球
//   const circle = new fabric.Circle({ radius: 40, fill: 'green', left: 100, top: 100 })

//   // 建立刪除按鈕
//   const deleteText = new fabric.Text('X', { fontSize: 20, fill: 'red', left: 60, top: -20 })

//   // Group：球 + 刪除按鈕
//   const group = new fabric.Group([circle, deleteText], { left: 100, top: 100 })

//   // 點擊刪除按鈕
//   deleteText.on('mousedown', function () {
//     canvas.remove(group)
//   })
